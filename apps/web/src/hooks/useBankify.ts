...
const useBankify = () => {
    ...
    const [nostrState, setNostrState] = useState<any>({
     ...
     useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (!window?.blindSigJS || !window?.super_nostr) {
    ...
    const sendLN = async () => {
        if (!Object.keys(nostrState.nwc_info).length) return alert('Please create an NWC connection first.');
        const mymint = nostrState.mymint;
        const appPubkey = Object.keys(nostrState.nwc_info)[0];
        const invoice = prompt('Enter a Lightning Network invoice');
        const invoiceAmt = bolt11.decode(invoice).satoshis;
        if (!invoiceAmt) return alert('Amountless invoices are not yet supported by this wallet.');
        const amntForAmountlessInvoice = null;
        ...
        const reply = await send(mymint, invoice, amntForAmountlessInvoice, appPubkey);
        alert(reply);
    };
